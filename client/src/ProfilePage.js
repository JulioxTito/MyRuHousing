...
function ProfilePage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        // fetch profile info
        const res = await fetch(`https://myruhousing.onrender.com/api/profile?uid=${user.uid}`);
        if (res.ok) {
        ...
        setPhotoUrl(savedPhoto || `https://myruhousing.onrender.com/api/profile-photo/${user.uid}?${Date.now()}`);
      } else {
  ...
  const handlePhotoUpload = async () => {
    ...
    const res = await fetch("https://myruhousing.onrender.com/api/profile-photo", {
      method: "POST",
      body: formData,
    });
    if (res.ok) {
      const newUrl = `https://myruhousing.onrender.com/api/profile-photo/${userId}?${Date.now()}`;
      setPhotoUrl(newUrl);
  ...
  const handleSave = async () => {
    ...
    const res = await fetch("https://myruhousing.onrender.com/api/profile", {
      method: "POST",
    ...
    if (res.ok) {
      ...
      const updatedProfile = await fetch(`https://myruhousing.onrender.com/api/profile?uid=${firebaseUser.uid}`);
